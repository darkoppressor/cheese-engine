        ...
        for (packet = peer->Receive(); packet != 0; peer->DeallocatePacket(packet), packet = peer->Receive()) {
            ...
            switch (packet_id) {
                    ...
                    if (status == "server") {
                        ...
                        if (client != 0) {
                            if (client->name.length() > 0) {
                                string msg = client->name + " (" + packet->systemAddress.ToString(true) +
                                             ") has disconnected";
                ...
                case ID_IP_RECENTLY_CONNECTED:
                    ...
                    if (status == "client") {
                        Log::add_log("Connected too recently to server: " +
                                     string(packet->systemAddress.ToString(true)));
                ...
                default:
                    ...
                    if (!receive_game_packet(packet, packet_id)) {
                        Log::add_log("Received message with unknown identifier: " +
                                     Strings::num_to_string((int) packet_id));
                    }
...
string Network_Engine::get_stats () {
    ...
    if (status != "off") {
        str_stats += "In: " + Strings::num_to_string((long double) stat_bytes_received / (long double) 1000.0) +
                     " kB/s\n";
