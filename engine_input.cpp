...
void Engine_Input::prepare_for_input () {
    ...
    if (Engine::allow_screen_keyboard() && !Game_Window::is_screen_keyboard_shown() &&
        Engine::mutable_info_selected()) {
            ...
            if (!event_consumed) {
                ...
                if (Engine::mutable_info_selected()) {
                    ...
                    for (int i = 0; i < lines.size(); i++) {
                        ...
                        if (i < lines.size() - 1 && Engine::ptr_mutable_info->allows_input("newline") &&
                            Engine::ptr_mutable_info->text.length() < Engine::ptr_mutable_info->max_text_length) {
                            input_newline();
                        }
                    }
                }
    ...
    switch (event.type) {
        ...
        case SDL_CONTROLLERAXISMOTION:
            ...
            if (!event_ignore_command_set) {
                ...
                if (!event_consumed && Window_Manager::is_any_window_open() && !Engine::console.on &&
                    !Engine::mutable_info_selected() && GUI_Manager::gui_axis_nav_last_direction != "up" &&
                    event.caxis.axis == SDL_CONTROLLER_AXIS_LEFTY &&
                    event.caxis.value < -Engine_Data::controller_dead_zone) {
                ...
                if (!event_consumed && Window_Manager::is_any_window_open() && !Engine::console.on &&
                    !Engine::mutable_info_selected() && GUI_Manager::gui_axis_nav_last_direction != "down" &&
                    event.caxis.axis == SDL_CONTROLLER_AXIS_LEFTY &&
                    event.caxis.value > Engine_Data::controller_dead_zone) {
            ...
            if (!event_ignore_command_set) {
                if (Engine::mutable_info_selected() && !Engine::allow_screen_keyboard()) {
                    if (!Engine_Data::controller_text_entry_small) {
                        if (!event_consumed && Engine::ptr_mutable_info->allows_input("newline") &&
                            Engine::ptr_mutable_info->text.length() < Engine::ptr_mutable_info->max_text_length &&
                        ...
                        if (!event_consumed && Engine::is_console_selected() &&
                            event.cbutton.button == SDL_CONTROLLER_BUTTON_LEFTSTICK) {
                            Engine::console.do_command();

                            event_consumed = true;
                        } else if (!event_consumed && Engine::is_chat_selected() &&
                                   event.cbutton.button == SDL_CONTROLLER_BUTTON_LEFTSTICK) {
                        ...
                        if (!event_consumed && Engine::ptr_mutable_info->allows_input("backspace") &&
                            event.cbutton.button == SDL_CONTROLLER_BUTTON_LEFTSHOULDER &&
                            Engine::ptr_mutable_info->text.length() > 0) {
                            input_backspace();
                        ...
                        if (!event_consumed && event.cbutton.button == SDL_CONTROLLER_BUTTON_B) {
                            if (Engine::ptr_mutable_info->allows_input("backspace") &&
                                Engine::ptr_mutable_info->text.length() > 0) {
                                input_backspace();
                            } else {
                ...
                if (!event_consumed && (Engine::mutable_info_selected() || Window_Manager::is_any_window_open()) &&
                    event.cbutton.button == SDL_CONTROLLER_BUTTON_B) {
                    GUI_Manager::gui_nav_back_controller();
                ...
                if (!event_consumed && Window_Manager::is_any_window_open() && !Engine::console.on &&
                    ...
                    Controller_Manager::controller_state(Controller_Manager::CONTROLLER_ID_ALL,
        ...
        case SDL_KEYDOWN:
            ...
            if (!event_ignore_command_set) {
                if (Engine::mutable_info_selected()) {
                    ...
                    if (!event_consumed && event.key.repeat == 0 && Engine::is_console_selected() &&
                        (event.key.keysym.scancode == SDL_SCANCODE_RETURN ||
                         event.key.keysym.scancode == SDL_SCANCODE_KP_ENTER)) {
                        Engine::console.do_command();

                        event_consumed = true;
                    } else if (!event_consumed && event.key.repeat == 0 && Engine::is_chat_selected() &&
                               (event.key.keysym.scancode == SDL_SCANCODE_RETURN ||
                                event.key.keysym.scancode == SDL_SCANCODE_KP_ENTER)) {
                        Engine::chat.send_chat();
                    ...
                    if (!event_consumed && !Engine::is_console_selected() &&
                        Engine::ptr_mutable_info->allows_input("space") &&
                ...
                if (event.key.repeat == 0) {
                    ...
                    // Toggle fullscreen
                    if (!event_consumed && (keystates[SDL_SCANCODE_LALT] || keystates[SDL_SCANCODE_RALT]) &&
                        (event.key.keysym.scancode == SDL_SCANCODE_RETURN ||
                         event.key.keysym.scancode == SDL_SCANCODE_KP_ENTER)) {
                    ...
                    if (!event_consumed &&
                        (Window_Manager::is_any_window_open() || Engine::console.on || Engine::chat.on) &&
                ...
                // Paste
                if (!event_consumed && (keystates[SDL_SCANCODE_LCTRL] || keystates[SDL_SCANCODE_RCTRL]) &&
...
void Engine_Input::handle_input_states () {
    ...
    if (axis_left_x >= -Engine_Data::controller_dead_zone && axis_left_x <= Engine_Data::controller_dead_zone &&
        axis_left_y >= -Engine_Data::controller_dead_zone && axis_left_y <= Engine_Data::controller_dead_zone) {
