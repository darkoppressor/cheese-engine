    ...
    for (int i = 0; i < SDL_GetNumTouchDevices(); i++) {
        ...
        if (touch_device_id != 0) {
            for (int j = 0; j < SDL_GetNumTouchFingers(touch_device_id); j++) {
                ...
                if (touch_finger != 0) {
                    vector<SDL_GameControllerButton> touch_buttons = check_for_button_press(touch_finger->x,
                                                                                            touch_finger->y);
...
vector<SDL_GameControllerButton> Touch_Controller::check_for_button_press (float x, float y) {
    ...
    Collision_Circ<double> circle_touch(((float) touch_x - offset_x) / scale_x, ((float) touch_y - offset_y) / scale_y,
                                        Engine_Data::touch_finger_size);
...
void Touch_Controller::render () {
    ...
    Render::render_texture(anchor_dpad.x * scale_x - anchor_dpad.r * scale_mean,
                           ...
                           Options::touch_controller_opacity, scale_anchor_dpad * scale_mean,
                           scale_anchor_dpad * scale_mean);
