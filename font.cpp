            ...
            if(X+get_letter_width()*scale_x>=0 && X<=Game_Window::width() && Y+get_letter_height()*scale_y>=0 && Y<=Game_Window::height()){
                ...
                if(!allowed_area_present || (allowed_area_present && X>=allowed_area.x && X+get_letter_width()*scale_x<=allowed_area.x+allowed_area.w && Y>=allowed_area.y && Y+get_letter_height()*scale_y<=allowed_area.y+allowed_area.h)){
                    if(shadow_distance!=0 && Options::font_shadows){
                        //Render the shadow
                        Render::render_sprite(X+shadow_distance,Y+shadow_distance,Image_Manager::get_image(sprite.name),&chars[ascii],opacity,scale_x,scale_y,angle,"ui_black");
                    }
                    ...
                    Render::render_sprite(X,Y,Image_Manager::get_image(sprite.name),&chars[ascii],opacity,scale_x,scale_y,angle,character_color);
                }
            }
