...
void Console::run_commands (const vector<string>& command_list) {
    for (int n = 0; n < command_list.size(); n++) {
        ...
        if (command.length() > 0) {
            if (command == "commands") {
                ...
                text += "\nEngine version: " + Engine_Version::get_engine_version() + " " +
                        Engine_Version::get_engine_status() + " (updated on " + Engine_Version::get_engine_date() + ")";
                add_text(text);
        ...
        } else if (info_input.text.length() >= 2 &&
                   (isdigit(info_input.text[0]) || info_input.text[0] == '-' || info_input.text[0] == 'd')) {
            ...
            if (dice_command.size() == 2 && Strings::is_number(dice_command[0]) &&
                Strings::is_number(dice_command[1])) {
