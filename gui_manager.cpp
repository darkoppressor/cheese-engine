...
void GUI_Manager::gui_nav_back_android(){
    ...
    if(Engine::mutable_info_selected()){
        ...
        if(!Game_Manager::in_progress && Window_Manager::is_window_open(window) && Window_Manager::open_window_count()==1){
            Engine::quit();
        }
        else{
...
void GUI_Manager::render_gui_selector(){
    ...
    if(object_pos.x!=-1){
        if(Engine_Data::gui_selector_style=="standard" || Engine_Data::gui_selector_style=="corners" || Engine_Data::gui_selector_style=="chasers"){
