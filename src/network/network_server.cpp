...
bool Network_Server::start_as_server (bool allow_clients) {
    if (Network_Engine::status == "off") {
        ...
        if (startup == RakNet::RAKNET_STARTED) {
            ...
            Network_Engine::clients.push_back(Client_Data(Network_Engine::id, Network_Engine::address, Options::name,
                                                          true));
...
void Network_Server::toggle_client_spectator (uint32_t client_index) {
    if (Network_Engine::status == "server") {
        if (Network_Engine::clients.size() > 0 && client_index < Network_Engine::clients.size()) {
            if (!Network_Engine::clients[client_index].spectator ||
                Network_Engine::get_player_count() < Options::max_players) {
...
void Network_Server::send_version () {
    if (Network_Engine::status == "server") {
        ...
        Network_Engine::peer->Send(&bitstream, MEDIUM_PRIORITY, RELIABLE_ORDERED, ORDERING_CHANNEL_CONNECTION,
                                   Network_Engine::packet->guid, false);
    }
}
    ...
    if (Network_Engine::status == "server") {
        ...
        for (int i = 0; i < Network_Engine::clients.size(); i++) {
            ...
            if (client->connected) {
                ...
                Network_Engine::stat_counter_bytes_sent += bitstream.GetNumberOfBytesUsed();
                Network_Engine::peer->Send(&bitstream, HIGH_PRIORITY, RELIABLE_ORDERED, ORDERING_CHANNEL_CLIENT_LIST,
                                           client->id, false);
            }
        }
    }
        ...
        if (Game_Manager::in_progress) {
            for (int i = 1; i < Network_Engine::clients.size(); i++) {
                if (Network_Engine::clients[i].connected) {
                    ...
                    if (Network_Engine::clients[i].updates_this_second < client_rate_updates &&
                        ++Network_Engine::clients[i].counter_update >= Engine::UPDATE_RATE / client_rate_updates) {
...
void Network_Server::send_update (Client_Data* client, uint32_t client_rate_bytes) {
    if (Network_Engine::status == "server") {
        ...
        if (client->bytes_this_second + stream_bytes <= client_rate_bytes) {
            ...
            Network_Engine::peer->Send(&bitstream, HIGH_PRIORITY, UNRELIABLE, ORDERING_CHANNEL_UNORDERED, client->id,
                                       false);
        }
    }
}
...
void Network_Server::send_ping_list () {
    if (Network_Engine::status == "server") {
        ...
        Network_Engine::stat_counter_bytes_sent += bitstream.GetNumberOfBytesUsed();
        Network_Engine::peer->Send(&bitstream, LOW_PRIORITY, RELIABLE_ORDERED, ORDERING_CHANNEL_CHAT,
...
void Network_Server::send_paused () {
    if (Network_Engine::status == "server") {
        ...
        Network_Engine::stat_counter_bytes_sent += bitstream.GetNumberOfBytesUsed();
        Network_Engine::peer->Send(&bitstream, HIGH_PRIORITY, RELIABLE_ORDERED, ORDERING_CHANNEL_PAUSED,
                                   RakNet::UNASSIGNED_RAKNET_GUID, true);
    }
}
...
void Network_Server::send_sound (string sound, RakNet::RakNetGUID target_id, bool broadcast) {
    if (Network_Engine::status == "server") {
        ...
        Network_Engine::stat_counter_bytes_sent += bitstream.GetNumberOfBytesUsed();
        Network_Engine::peer->Send(&bitstream, HIGH_PRIORITY, RELIABLE_ORDERED, ORDERING_CHANNEL_SOUND, target_id,
                                   broadcast);
    }
}
