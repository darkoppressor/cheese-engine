...
class Custom_Sound {
    ...
    public:
        // A note length string can be either one of the strings listed below OR a double value representing the length
        // in seconds
        ...
        // Frequency strings can represent either a single frequency, or a set of frequencies added together to form a
        // chord
        // To form a chord, simply combine multiple frequencies into a single frequency string with plus sign separators
        // Each individual frequency is represented by a capital letter A to G, a number 0 to 8, and an optional
        // accidental (symbols explained below)
        // Additionally, for convenience, a frequency consisting of only the capital letter component will act as a
        // shortcut to the corresponding letter in octave 5
        ...
        // If fewer values are passed than channels exist, the last passed value will be used for all subsequent
        // channels
        ...
        // Pass an empty string for frequency_string to use the currently set frequency string for each channel. Pass a
        // frequency string to use it instead (for all channels)
        // Pass an empty string for length_string to use the currently set length string. Pass a length string to use it
        // instead
        // Pass "off" for waveform_override to use the currently set waveforms for each channel. Pass a waveform string
        // to specify a different waveform (for all channels)
        // Pass a negative number for volume_override to use the currently set volumes for each channel. Pass a positive
        // volume value (including 0.0) to specify a different volume (for all channels)
        void add_note(std::string frequency_string, std::string length_string, std::string waveform_override = "off",
                      double volume_override = -1.0);
