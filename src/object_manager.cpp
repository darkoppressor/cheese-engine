    ...
    if (configure_command != -1) {
        ...
        switch (Engine_Input::event.type) {
            case SDL_CONTROLLERBUTTONDOWN:
                ...
                if (!event_consumed &&
                    (Engine_Input::event.cbutton.button == SDL_CONTROLLER_BUTTON_START || allow_keys_and_buttons)) {
                    if (Engine_Input::event.cbutton.button != SDL_CONTROLLER_BUTTON_START) {
                        if (configure_command < game_commands.size()) {
                            game_commands[configure_command].button =
                                (SDL_GameControllerButton) Engine_Input::event.cbutton.button;
                        }
            ...
            case SDL_KEYDOWN:
                ...
                if (!event_consumed && Engine_Input::event.key.repeat == 0 &&
                    (Engine_Input::event.key.keysym.scancode == SDL_SCANCODE_ESCAPE ||
                     Engine_Input::event.key.keysym.scancode == SDL_SCANCODE_AC_BACK || allow_keys_and_buttons)) {
                    if (Engine_Input::event.key.keysym.scancode != SDL_SCANCODE_ESCAPE &&
                        Engine_Input::event.key.keysym.scancode != SDL_SCANCODE_AC_BACK &&
                        Engine_Input::event.key.keysym.scancode != SDL_SCANCODE_MENU) {
                        if (configure_command < game_commands.size()) {
