...
void Information::set_dimensions () {
    ...
    if (scrolling || (scroll_width > 0 && scroll_height > 0)) {
        ...
        if (text_mutable) {
            ...
            w = Engine_Data::gui_border_thickness * 2.0 + Strings::longest_line(text) * ptr_font->spacing_x +
                ptr_font->get_gui_padding_x();
            h = Engine_Data::gui_border_thickness * 2.0 + Strings::newline_count(text) * ptr_font->spacing_y +
                (Strings::newline_count(text) + 1) * ptr_font->get_letter_height() + ptr_font->get_gui_padding_y();
        }
    }
}
...
void Information::scroll_up (int y_offset) {
    if (scrolling) {
        if (y_offset + y + Object_Manager::get_font(font)->spacing_y * scroll_offset <
            y_offset + y + h - Object_Manager::get_font(font)->spacing_y * 3) {
            scroll_offset += 1;
        }
    }
}
...
void Information::scroll_down (int y_offset) {
    if (scrolling) {
        if (y_offset + y + Object_Manager::get_font(font)->spacing_y * scroll_offset +
            Object_Manager::get_font(font)->spacing_y * Strings::newline_count(text) > y_offset + y) {
            scroll_offset -= 1;
        }
    }
}
...
void Information::scroll_to_cursor () {
    if (scrolling) {
        ...
        if (cursor_position > -1) {
            ...
            if (text.length() > 0 && cursor_position < (int) text.length() &&
    ...
    if (tooltip_text.length() > 0) {
        ...
        if (Engine::mouse_allowed() && GUI_Manager::gui_mode == "mouse" && Collision::check_rect(box_a, box_b)) {
            Tooltip::setup(tooltip_text, mouse_x, mouse_y);
                   ...
                   GUI_Manager::is_gui_object_selected(this)) {
                ...
                if (text_mutable) {
                    if (Collision::check_rect(box_a, box_b)) {
                        if (text.length() > 0) {
                            ...
                            for (int i = top_line, line_on_screen = 0; !character_clicked && i < bottom;
                                 i++, line_on_screen++) {
                                Collision_Rect<double> box_line(x_offset + x,
                                                                y_offset + y + ptr_font->spacing_y * line_on_screen,
                                                                ptr_font->spacing_x * line_width, ptr_font->spacing_y);
...
void Information::render (int x_offset, int y_offset) {
    ...
    if (sprite.name.length() > 0 && background_type == "sprite") {
        ...
        if (Engine::current_color_theme()->information_border != "<INVISIBLE>") {
            Render::render_rectangle(x_offset + x, y_offset + y, w, h, background_opacity,
                                     Engine::current_color_theme()->information_border);
        }
    ...
    if (text.length() > 0 || text_mutable) {
        ...
        if (font_color_real != "<INVISIBLE>") {
            ptr_font->show(x_offset + x + Engine_Data::gui_border_thickness,
                           y_offset + y + Engine_Data::gui_border_thickness + scrolling_adjust_y, text, font_color_real,
                           1.0, 1.0, 1.0, 0.0, allowed_area, text_character_colors);
