...
int Window::get_scrolling_button_offset (int index) {
    ...
    int position_adjust_y = buttons[index].h * get_scrolling_button_position(index) +
                            Engine_Data::scrolling_buttons_spacing * get_scrolling_button_position(index);
...
bool Window::is_scrolling_button_on_screen (int index) {
    ...
    if (x + buttons[index].x >= allowed_area.x &&
        x + buttons[index].x + buttons[index].w <= allowed_area.x + allowed_area.w && button_y >= allowed_area.y &&
        button_y + buttons[index].h <= allowed_area.y + allowed_area.h) {
        return true;
    } else {
        return false;
    }
...
void Window::scroll_up () {
    if (y + buttons[last_normal_button + 1].h * scroll_offset + Engine_Data::scrolling_buttons_spacing* scroll_offset <
        y + h - buttons[last_normal_button + 1].h * 3 - Engine_Data::scrolling_buttons_spacing*3) {
        scroll_offset += 1;
    }
}

void Window::scroll_down () {
    if (y + buttons[last_normal_button + 1].h * scroll_offset + buttons[last_normal_button + 1].h *
        (buttons.size() - 2 - last_normal_button) > y) {
        scroll_offset -= 1;
    }
}
            ...
            switch (Engine_Input::event.type) {
                case SDL_MOUSEBUTTONDOWN:
                    ...
                    if (!event_consumed && Engine_Input::event.button.button == SDL_BUTTON_LEFT) {
                        ...
                        Collision_Rect<double> box_a(mouse_x, mouse_y, Engine_Data::cursor_width,
                                                     Engine_Data::cursor_height);
                ...
                case SDL_MOUSEWHEEL:
                    ...
                    if (!event_consumed && Engine_Input::event.wheel.y < 0) {
                        if (scrolling_buttons.length() > 0) {
                            Collision_Rect<double> box_a(mouse_x, mouse_y, Engine_Data::cursor_width,
                                                         Engine_Data::cursor_height);
                            Collision_Rect<double> box_b(x, y + Engine_Data::scrolling_buttons_offset, w,
                                                         h - Engine_Data::scrolling_buttons_offset - 6);
