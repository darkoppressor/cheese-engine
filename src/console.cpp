...
void Console::recall_up () {
    if (current_recalled_command_string > 0 &&
        recalled_command_strings[current_recalled_command_string - 1] != "\x1F") {
                ...
                if (valid_commands.size() == 1) {
                    ...
                    while (!mismatch_found) {
                        ...
                        for (int i = 0; i < valid_commands.size(); i++) {
                            if (valid_commands[i].length() < shared_characters.length() ||
                                !boost::algorithm::starts_with(valid_commands[i], shared_characters)) {
                    ...
                    if (info_input.text.length() == shared_characters.length() &&
                        last_tab_complete_command.length() == 0) {
                        last_tab_complete_command = shared_characters;
                    } else if (info_input.text.length() == shared_characters.length() &&
                               last_tab_complete_command.length() > 0) {
                        ...
                        for (int i = 0; i < valid_commands.size(); i++) {
                            ...
                            if (valid_command_display.back().length() + this_line.length() >
                                info_display.w / Object_Manager::get_font(info_display.font)->spacing_x) {
                                valid_command_display.push_back("");
                            }
        ...
        switch (Engine_Input::event.type) {
            ...
            case SDL_KEYDOWN:
                ...
                if (Engine_Input::event.key.repeat == 0) {
                    if ((Engine_Input::event.key.keysym.scancode == SDL_SCANCODE_GRAVE &&
                         !keystates[SDL_SCANCODE_LSHIFT] && !keystates[SDL_SCANCODE_RSHIFT]) ||
                        Engine_Input::event.key.keysym.scancode == SDL_SCANCODE_AC_SEARCH) {
            ...
            if (y < Game_Window::height()) {
                ...
                for (int i = 0; i < info_temp_display.text.length(); i++) {
                    if (Strings::is_newline_character(info_temp_display.text[i])) {
                        ...
                        if (!text_timers[line].is_started() ||
                            (text_timers[line].is_started() && text_timers[line].get_ticks() >= line_timeout)) {
