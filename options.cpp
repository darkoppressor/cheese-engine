...
string Options::get_option_description (string option) {
    if (option == "cl_logic_update_rate") {
        ...
        return
            "the display number that the game's window should be placed on\n - changes to this option are only applied when the UI is reloaded with \"reload\"";
    } else if (option == "cl_fullscreen_state") {
        return
            "enable or disable the fullscreen status of the window\n - changes to this option are only applied when the UI is reloaded with \"reload\"";
    } else if (option == "cl_fullscreen_mode") {
        return
            "the mode used when the window is made fullscreen\n - changes to this option are only applied when the UI is reloaded with \"reload\"\n - valid values: \"standard\", \"desktop\", \"windowed\"";
    } else if (option == "cl_vsync") {
        return "enable vertical synchronization\n - changes to this option are only applied when the game is restarted";
    } else if (option == "cl_accelerometer_controller") {
        return
            "allow the Android accelerometer to be used as a game controller\n - changes to this option are only applied when the game is restarted";
    } else if (option == "cl_touch_controller_state") {
...
void Options::apply_options_graphics (const string& cl_screen_width, const string& cl_screen_height,
                                      const string& cl_display_number, const string& cl_fullscreen_state,
                                      const string& cl_fullscreen_mode, const string& cl_vsync, const string& cl_fps,
                                      const string& cl_hw_cursor, const string& cl_font_shadows) {
    ...
    if (old_screen_width != cl_screen_width || old_screen_height != cl_screen_height ||
        old_display_number != cl_display_number || old_fullscreen_state != cl_fullscreen_state ||
...
void Options::apply_options_input (const string& cl_bind_cursor, const string& cl_screen_keyboard,
                                   const string& cl_accelerometer_controller, const string& cl_touch_controller_state,
                                   const string& cl_touch_controller_opacity) {
...
bool Options::save_options () {
    ...
    save << "\tversion:" << Engine_Version::get_version() << "\n";
