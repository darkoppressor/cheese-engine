    ...
    if (!moused_over && !clicked) {
        ...
        if (sprite.name.length() > 0) {
            ...
            if (Engine::current_color_theme()->button_border != "<INVISIBLE>") {
                Render::render_rectangle(x_offset + x, y_offset + y, w, h, 1.0,
                                         Engine::current_color_theme()->button_border);
            }
            ...
            if (font_color_real != "<INVISIBLE>") {
                ...
                if (text_sprite.name.length() > 0) {
                    ...
                    text_sprite.render(x_offset + x + Engine_Data::gui_border_thickness,
                                       y_offset + y + h / 2.0 - text_sprite.get_height() / 2.0);
                }
